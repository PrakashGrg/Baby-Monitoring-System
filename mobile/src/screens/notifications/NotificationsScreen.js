...
const TYPE_CONFIG = {
  cry:         { icon: 'volume-high',     color: colors.accent,   label: 'Crying' },
  motion:      { icon: 'walk',            color: colors.amber,    label: 'Motion' },
  temperature: { icon: 'thermometer',     color: colors.danger,   label: 'Temperature' },
  humidity:    { icon: 'water',           color: colors.info,     label: 'Humidity' },
  sleep:       { icon: 'moon',            color: colors.primary,  label: 'Sleep' },
  summary:     { icon: 'bar-chart',       color: colors.green,    label: 'Summary' },
};
          ...
          return (
            ...
            >
              <View style={[styles.notifIcon, { backgroundColor: cfg.color + '20' }]}>
                <Ionicons name={cfg.icon + '-outline'} size={22} color={cfg.color} />
              </View>
